...
public class ImpactPulseVFX : VFXBase
{
    [SerializeField] private float duration = 0.35f;
    [SerializeField] private float startScale = 0.35f;
    [SerializeField] private float endScale = 1.35f;
    [SerializeField] private Color startColor = new(1f, 0.58f, 0.18f, 1f);
    [SerializeField] private Color endColor = new(1f, 0.1f, 0.02f, 0f);
    ...
    private void Update()
    {
        ...
        if (billboardToCamera && Camera.main != null) transform.forward = Camera.main.transform.forward;
    ...
    public override void Play(VFXRequest request)
    {
        ...
        if (request.Direction.sqrMagnitude > 0f) transform.rotation = Quaternion.LookRotation(request.Direction);
    ...
    private void EnsureQuadMesh()
    {
        ...
        var mesh = new Mesh { name = "ElementLine_Quad" };
