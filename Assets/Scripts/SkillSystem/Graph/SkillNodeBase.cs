...
public abstract class SkillNodeBase : ScriptableObject
{
    ...
    // ============================================================
    //  【新增】编译支持 —— 混合架构：节点可生成 SkillEffectData
    // ============================================================

    /// <summary>
    ///     节点是否可被编译为 SkillEffectData。
    ///     叶子节点（Damage/VFX/Buff）返回 true，控制流节点（Condition/Loop）返回 false。
    /// </summary>
    public virtual bool CanCompile => false;

    /// <summary>
    ///     将节点编译为 SkillEffectData 列表。
    ///     只有 CanCompile == true 的节点需要实现此方法。
    ///     返回 null 表示编译失败，SkillBuilder 会标记为动态节点。
    /// </summary>
    public virtual List<SkillEffectData> Compile(SkillContext ctx = null)
    {
        return null;
    }
    ...
    /// <summary>
    ///     获取本节点对时间轴的贡献（秒）。
    ///     例如 DelayNode 返回 delaySeconds，其他节点返回 0。
    ///     用于 SkillBuilder 计算时间游标。
    /// </summary>
