...
public class PlayVFXNode : SkillNodeBase
{
    ...
    {
        string resolvedKey = ResolveVfxKey(ctx);
        if (string.IsNullOrWhiteSpace(resolvedKey))
            resolvedKey = vfxKey.Resolve(ctx);

        var targetMode = parentBinding == TransformBinding.Caster
            ? SkillEffectTargetMode.Caster
            : SkillEffectTargetMode.PrimaryTarget;

        var effect = SkillEffectData.CreateVFX(
            resolvedKey,
            targetMode,
            styleKey.Resolve(ctx),
            scaleMultiplier.Resolve(ctx),
            parentBinding != TransformBinding.World,
            ResolveDirection(ctx),
            lengthOverride.Resolve(ctx),
            widthMultiplier.Resolve(ctx)
        );

        effect.Duration = durationOverride.Resolve(ctx);
        effect.BaseValue = intensityMultiplier.Resolve(ctx);
